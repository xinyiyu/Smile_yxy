...
# calculate a list of quality score and save
images = glob.glob(f'{sys.argv[1]}/*.jpg')
qualities = pd.DataFrame(columns=['seller_ID', 'quality'])
qualities.seller_ID = [image.split('/')[-1].split('.')[0] for image in images]
for i, image in enumerate(images):
    qualityscore = test_measure_BRISQUE(image)
    qualities.iloc[i, 1] = round(qualityscore, 2)
    if i % 100 == 0:
        print(f'{i} / {len(images)}')
qualities = qualities.sort_values(by='seller_ID').reset_index(drop=True)
qualities.to_csv(sys.argv[2], sep='\t', index=False)
