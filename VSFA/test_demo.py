...
if __name__ == "__main__":
    ...
    parser.add_argument('--video_height', type=int, default=None,
                        help='video height')
    parser.add_argument('--exclude_frames', type=int, default=300,
    ...
    if args.video_format == 'YUV420':
        video_data = skvideo.io.vread(args.video_path, args.video_height, args.video_width, args.num_frames, inputdict={'-pix_fmt': 'yuvj420p'})
    else:
        video_data = skvideo.io.vread(args.video_path, num_frames=args.num_frames)
    
    video_data = video_data[args.exclude_frames:args.num_frames]
    ...
    with open('test_quality.txt', 'a') as f:
        f.write(f"{args.video_path.split('/')[-1].split('.')[0]}\t{y_pred}\n")
